...
class Sql():
    ...
    def execute(self,query,param=None):
        ...
        try:
            ...
            #self.escribirLog_config("[Error] " + msg) #se quita dado que causa errores cuando existe una exception (Problemas para ejecutar en sql FATAL:  lo siento, ya tenemos demasiados clientes)
            conection.commit()            
            cur.close()
            conection.close() 
    ...
    def escribirLog_config(self,log):
        va = "INSERT INTO public.conf_logs (log) VALUES (" + "'" + self.curTime() + ' '  + log + "'" + ")"
        self.execute(va) 
        return "ok"    
